...
class SongRepository:
    ...
    async def get_song_by_ids(session: AsyncSession, ids: list[int]):
        result = await session.exec(select(Song).where(Song.id.in_(ids)))
        return result.all()

    @staticmethod
    async def get_songs_by_level(session: AsyncSession, level_id: int):
        result = await session.exec(select(Song).where(Song.level_id == level_id))
        return result.all()
    ...
    async def get_all_click_counts(session: AsyncSession):
        ...
        return {song_id: int(total) for song_id, total in rows.all()}
